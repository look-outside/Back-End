...
public interface ArticleRepository extends JpaRepository<Article, Integer>, JpaSpecificationExecutor<Article>{
	...
	@Query(value = "select a.*, u.useNick "
			+ "from ebdb.Article a "
			+ "join ebdb.User u on a.useNo = u.useNo "
			+ "where a.artNo = ?1 ",
...
//	Page<Article> findByArtSubjectContaining(Pageable pageable,String keyword);//게시물 제목으로 검색
	...
	//전체 게시물 목록 조회 (관리자페이지)
	@Query(value = "select a.*, u.useNick, r.regAddr1, r.regAddr2, i.imgSave, i.imgPath "
			+ "from ebdb.Article a "
			+ "join ebdb.Region r on a.regNo = r.regNo "
			+ "join ebdb.User u on a.useNo = u.useNo "
			+ "join ebdb.ArticleImg i on a.artNo = i.artNo "
			+ "where i.imgNo = 1",
			countQuery ="select a.*, u.useNick, r.regAddr1, r.regAddr2, i.imgSave, i.imgPath "
					+ "from ebdb.Article a "
					+ "join ebdb.Region r on a.regNo = r.regNo "
					+ "join ebdb.User u on a.useNo = u.useNo "
					+ "join ebdb.ArticleImg i on a.artNo = i.artNo "
